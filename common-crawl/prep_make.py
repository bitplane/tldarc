...
def generate_makefile(matches):
    ...
    print()
    print(".PHONY: all latest combined clean-crawls stats")
    print()
    ...
    # Latest target - just the most recent crawl
    if sorted_collections:
        latest_name, _ = sorted_collections[0]
        print(f"latest: {latest_name}.tsv.gz")
        print()
    
    # Combined target - merges all crawls into a single release file
    if sorted_collections:
        latest_name, _ = sorted_collections[0]
        all_gz_files = " ".join([f"{name}.tsv.gz" for name, _ in sorted_collections])
        combined_name = f"commoncrawl-all-{latest_name}"
        print(f"# Combined release target")
        print(f"{combined_name}.tsv: {all_gz_files} merge_domains.py")
        print(f"\t@echo 'Merging newer files into $@...'")
        print(f"\t@for f in $$(ls -t *.tsv.gz 2>/dev/null | tac); do \\")
        print(f"\t\tif [ \"$$f\" -nt \"$@\" ]; then \\")
        print(f"\t\t\techo \"Processing $$f\"; \\")
        print(f"\t\t\tzcat \"$$f\" | LC_ALL=C sort | ./merge_domains.py $@; \\")
        print(f"\t\t\ttouch -r \"$$f\" $@; \\")
        print(f"\t\tfi; \\")
        print(f"\tdone")
        print()
        print(f"# Special compression rule for combined files (no sort step)")
        print(f"commoncrawl-all-%.tsv.gz: commoncrawl-all-%.tsv")
        print(f"\tpigz -9 < $< > $@")
        print()
        print(f"combined: {combined_name}.tsv.gz")
        print()
    ...
    print("clean-crawls:")
    print("\trm -f *.tsv *.tsv.gz commoncrawl-all-*.tsv commoncrawl-all-*.tsv.gz")
    print()
    ...
    if sorted_collections:
        latest_name, _ = sorted_collections[0]
        combined_gz = f"commoncrawl-all-{latest_name}.tsv.gz"
        print(f"\t@if [ -f {combined_gz} ]; then echo 'Combined ({latest_name}): '`zcat {combined_gz} | wc -l`' domains'; fi")
